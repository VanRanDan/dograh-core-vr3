...
async def start_campaign(
    ...
    # Provisioned-only billing gate (resolve org via the campaign workflow's
    # owner, the same org the settlement emitter will settle against).
    org_id = await resolve_billing_org_id(campaign.workflow_id)
    quota_result = await check_and_reserve(org_id, workflow_id=campaign.workflow_id)
