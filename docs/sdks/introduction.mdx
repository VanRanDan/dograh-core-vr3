...
Generate an API key at [`/api-keys`](https://vr3ai.tapcloud.org/api-keys) (or `http://localhost:3010/api-keys` for self-hosted). See [API Keys](/configurations/api-keys) for details.
...
client = DograhClient(
    base_url="https://vr3ai.tapcloud.org",
...
const client = new DograhClient({
    baseUrl: "https://vr3ai.tapcloud.org",
    apiKey: "YOUR_API_KEY",
});
```
</CodeGroup>
