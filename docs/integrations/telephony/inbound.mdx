...
description: "Configure inbound calling for Twilio, Vonage, Plivo, Telnyx, Cloudonix, Vobiz, and Asterisk ARI providers in VR3 AI"
---

## Overview

VR3 AI supports inbound calling across all supported telephony providers. When someone calls your configured phone number, your voice agent automatically answers and handles the interaction. Inbound and outbound calls share the same telephony configuration.
...
VR3 AI exposes a **single inbound webhook URL** for the whole org:

```
https://vr3ai.tapcloud.org/api/v1/telephony/inbound/run
```
...
3. **Org & Agent Resolved**: VR3 AI identifies the org from the webhook's account credentials, then looks up which agent should answer based on the called number's **Inbound workflow** assignment in your telephony configuration
...
You don't construct per-workflow URLs anymore — the routing is done by the called number's inbound workflow assignment in VR3 AI.
...
- [Asterisk ARI Configuration](/integrations/telephony/asterisk-ari#configuration-in-VR3 AI)
...
This is what tells VR3 AI which agent to run when a call comes in for that number.

### Step 3: Point Your Provider at `/inbound/run`

Each provider needs its inbound webhook (Twilio Voice URL / Vonage Answer URL / etc.) set to the global VR3 AI endpoint:
...
<Note>
If you're self-hosting VR3 AI, replace `vr3ai.tapcloud.org` with your own domain.
</Note>
...
After completing both the VR3 AI configuration and provider-specific setup:
...
3. **Check Logs**: Monitor call logs in both VR3 AI and your provider's dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Provider mismatch error">
    ...
    - Ensure the phone number is configured for the same provider in both VR3 AI and your telephony console
  </Accordion>
  
  <Accordion title="Workflow not found error">
    - **Error**: "Workflow not found"
    - **Solution**: Open the called number in **/telephony-configurations** and confirm an **Inbound workflow** is assigned. Without an assignment, VR3 AI has no agent to route the call to.
  ...
  <Accordion title="Account validation failed">
    ...
    - **Solution**: Verify your account SID/credentials configuration in VR3 AI matches your telephony provider settings
  ...
  <Accordion title="Signature validation failed">
    ...
    - **Solution**: Verify your auth token configuration in VR3 AI matches your provider settings
  ...
  <Accordion title="Poor audio quality on inbound calls">
    - Check network bandwidth and latency from provider to VR3 AI
...
- **Monitoring**: Monitor both VR3 AI logs and provider dashboards for call analytics
