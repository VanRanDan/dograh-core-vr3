...
description: "Configure Twilio for voice communication in VR3 AI"
---

## Overview

Twilio is a cloud communications platform that enables voice calling, messaging, and video capabilities. VR3 AI's Twilio integration provides seamless connectivity for your voice agents.
...
Inbound routing is driven by the phone number itself — there is a **single webhook URL** for the entire org, and VR3 AI resolves which agent to run from the called number's assigned inbound workflow. **When you save an inbound workflow on a phone number, VR3 AI automatically pushes the webhook URL to that number's `VoiceUrl` in your Twilio account** (provided the credentials are correct and the number belongs to that account).
...
3. In the **Voice Configuration** section, confirm:
   - **Webhook** is set to: `https://vr3ai.tapcloud.org/api/v1/telephony/inbound/run`
   - **HTTP Method** is `POST`

   <Note>
     VR3 AI pushed this URL automatically when you saved the inbound workflow
     in Step 1. If the field is empty, shows a different URL, or VR3 AI
     ...
     `POST`, and click **Save Configuration**. On self-hosted VR3 AI,
...
3. Check call logs in both VR3 AI dashboard and Twilio Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls go to voicemail">
    ...
    exists in your VR3 AI telephony configuration and has an **Inbound
    workflow** assigned - Verify VR3 AI instance is running and responding
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    (Auth Token in VR3 AI matches Twilio Console) - Verify WebSocket connection
