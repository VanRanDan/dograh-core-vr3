---
title: "Agent Stream"
description: "Stream audio to a VR3 AI agent over a WebSocket"
---
...
`{agent_uuid}` is the agent's stable UUID (see [Get the Agent UUID](#get-the-agent-uuid) below). On self-hosted deployments, replace `vr3ai.tapcloud.org` with your backend host.
