...
description: "Configure Cloudonix for voice communication in VR3 AI"
---

## Overview

Cloudonix is a cloud communications platform that enables VR3 AI voice agents to connect to SIP communications
...
- A [Cloudonix account](https://cockpit.cloudonix.io/onboarding)
- A Cloudonix domain UUID (or the domain name)
- A Cloudonix domain API Key (Bearer Token)
- A Cloudonix **Voice Application** on that domain (optional — leave the field blank in VR3 AI and we'll auto-create one for you on save, with the application `url` pre-set)
- A Cloudonix outbound voice trunk service provider connection
- VR3 AI instance running and accessible

## Video Tutorial

Watch this step-by-step guide to set up Cloudonix with VR3 AI:

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.youtube.com/embed/qLKX0F99jpU?si=tA3zwCsNRKe-WCI1"
  title="VR3 AI Cloudonix Setup"
...
1. Log in to your [Cloudonix Console](https://cockpit.cloudonix.io/onboarding)
2. Find your **Domain ID** (UUID or domain name) and **Bearer Token** (Domain API Key) on the dashboard
3. (Optional) Navigate to your domain's **Applications** and create (or open) the application you'll use with VR3 AI, then copy its **Application Name**. Skip this if you want VR3 AI to auto-create the Voice Application for you on save.
...
4. Enter your credentials:
   - Bearer Token
   - Domain ID
   - Application Name — *optional*. Leave blank and VR3 AI will auto-create a Voice Application on save (with the application `url` and CXML runtime already configured) and store its name on this configuration.
...
6. Open the configuration you just created and add at least one **phone number** (with country code in E.164 format, e.g. `+1234567890`). The default caller ID is used for outbound calls.
   ...
   <Note>
     If VR3 AI auto-created the Voice Application for you, you still need to
...
Cloudonix routes inbound calls per **Voice Application** — the webhook URL is set once on the application, and applies to every DNID bound to it. **When you save an inbound workflow on a phone number, VR3 AI automatically pushes the webhook URL to your Voice Application's `url`** (provided the credentials are correct), so you don't need to set the webhook by hand. If VR3 AI auto-created the application during configuration save, the `url` is already set and this push is a no-op — you only need to bind your DNIDs to the auto-created application.
...
2. Create a new application (or open the existing one whose name you configured in VR3 AI) with these settings:
   - **Application Resource Type**: `Remote Application Resource`
   - **Application Runtime**: `Cloudonix (CXML)`
   - **Application URL**: `https://vr3ai.tapcloud.org/api/v1/telephony/inbound/run`
   - **HTTP Method**: `POST`
3. Under **DNID Numbers**, add each phone number (DNID) you want to route through this application
4. Save

   <Note>
     The Application URL is what VR3 AI's auto-push updates in Step 3 — you
     ...
     Either works. Self-hosted VR3 AI deployments use their backend domain
     in place of `vr3ai.tapcloud.org`.
   </Note>

   <img
     className="block dark:hidden"
     src="/images/cloudonix-inbound-config-application.png"
     alt="Cloudonix application form showing Remote Application Resource type, Cloudonix CXML runtime, the VR3 AI inbound URL, POST method, and a DNID number entry"
   />
   <img
     className="hidden dark:block"
     src="/images/cloudonix-inbound-config-application.png"
...
### Step 3: Assign an Inbound Workflow to the Phone Number in VR3 AI
...
2. Open the application whose name you configured in VR3 AI
3. Confirm:
   - **URL** is set to: `https://vr3ai.tapcloud.org/api/v1/telephony/inbound/run`
   - **Method** is `POST`

   <Note>
     VR3 AI pushed this URL automatically when you saved the inbound workflow
     in Step 3. The same URL is shared across every DNID bound to that
     application — VR3 AI routes each inbound call to the right agent based
     on the called number's inbound workflow assignment. If the field is
     empty, shows a different URL, or VR3 AI surfaced a sync warning on
     save, the auto-push failed — most often because the Bearer Token,
     Domain ID, or Application Name in VR3 AI is incorrect. Paste the URL
     into the field yourself, set the method to `POST`, and save. On
     self-hosted VR3 AI, replace `vr3ai.tapcloud.org` with your backend domain.
   </Note>
...
- Ensure your VR3 AI instance is publicly accessible
...
3. Check call logs in both VR3 AI dashboard and Cloudonix Console
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    ...
    configured in VR3 AI - Confirm the called number exists in your VR3 AI
    telephony configuration and has an **Inbound workflow** assigned -
    After assigning the inbound workflow, confirm VR3 AI successfully updated
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    /telephony-configurations - Verify the Bearer Token in VR3 AI matches the
...
- Use a dedicated Voice Application for VR3 AI so the shared `url` doesn't conflict with other systems
