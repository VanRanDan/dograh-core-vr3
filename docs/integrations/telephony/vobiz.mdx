...
description: "Configure Vobiz for voice communication in VR3 AI"
---

## Overview

Vobiz is a cloud-based telephony service provider that offers global reach with competitive pricing. VR3 AI's Vobiz integration provides reliable voice connectivity for your conversational AI agents.
...
- A Vobiz **Application** (optional — leave the field blank in VR3 AI and we'll auto-create one for you on save, with the `answer_url` pre-set)
...
3. (Optional) Navigate to the **Applications** section and create (or open) the application you'll use with VR3 AI, then copy its **Application ID**. Skip this if you want VR3 AI to auto-create the application for you on save.
...
3. Enter your credentials:
   - Auth ID
   - Auth Token
   - Application ID — *optional*. Leave blank and VR3 AI will auto-create a Vobiz Application on save (with the `answer_url` already configured) and store its Application ID on this configuration.
...
5. Open the configuration you just created and add at least one **phone number** (with country code in E.164 format, e.g. `+1234567890`). The default caller ID is used for outbound calls.
   ...
   <Note>
     If VR3 AI auto-created the Vobiz Application for you, you still need to
...
Vobiz numbers don't carry an `answer_url` directly — the URL lives on a Vobiz **Application**, and each number is linked to one application. **When you save an inbound workflow on a phone number, VR3 AI automatically pushes the webhook URL to your Vobiz Application's `answer_url`** (provided the credentials are correct). If VR3 AI auto-created the application during configuration save, the `answer_url` is already set and this push is a no-op.

### Step 1: Link the Phone Number to Your Vobiz Application

1. Log in to your Vobiz Console and open the **Applications** section
2. Edit the application whose ID you configured in VR3 AI
...
### Step 2: Assign an Inbound Workflow to the Phone Number in VR3 AI
...
2. Open the application whose ID you configured in VR3 AI
3. Confirm:
   - **Answer URL** is set to: `https://vr3ai.tapcloud.org/api/v1/telephony/inbound/run`
   ...
   <Note>
     ...
     Application ID in VR3 AI is incorrect. Paste the URL into the field
     yourself, set the method to `POST`, and save. On self-hosted VR3 AI,
     replace `vr3ai.tapcloud.org` with your backend domain.
   </Note>
...
3. Check call logs in both VR3 AI dashboard and Vobiz dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls go to voicemail or aren't answered">
    ...
    ID you configured in VR3 AI - Confirm the called number exists in your
    VR3 AI telephony configuration and has an **Inbound workflow** assigned
    - After assigning the inbound workflow, confirm VR3 AI successfully
    updated the application's `answer_url` (no warning shown on save) -
    Verify VR3 AI instance is running and responding
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    (Auth Token in VR3 AI matches Vobiz dashboard) - Verify WebSocket connection
...
- Use a dedicated Vobiz Application for VR3 AI so the shared `answer_url` doesn't conflict with other systems
