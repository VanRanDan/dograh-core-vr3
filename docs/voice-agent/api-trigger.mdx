...
If you are using the hosted version, replace `your-dograh-instance` with `vr3ai.tapcloud.org`.
...
The id is shown on each row in **Telephony configurations** (`https://vr3ai.tapcloud.org/telephony-configurations` for hosted or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
